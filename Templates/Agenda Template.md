<%*
const missionName = await tp.system.prompt("Agenda name (e.g. Outreach, Training)", "");
if (!missionName) {
  new Notice("Cancelled.");
  return;
}

const objective = await tp.system.prompt("Objective (specific, measurable)", "");
...
const endDateIso = await new Promise((resolve) => {
  ...
  function render() {
    ...
    const grid = modal.querySelector(".mission-calendar-grid");
    if (!grid) return;
    grid.innerHTML = "";
    ...
    for (let day = 1; day <= lastDay; day++) {
      ...
      btn.type = "button";
      if (isPast) btn.addClass("past");
      else if (isSelected) btn.addClass("selected");
      if (!isPast) {
  ...
  confirmBtn.addEventListener("click", () => { resolve(selectedIso); overlay.remove(); });
